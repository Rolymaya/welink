...
export class PlaygroundService implements OnModuleInit {
    ...
    async getPlaygroundSessionStatus(id: string, organizationId: string) {
    ...
    async disconnectPlaygroundSession(id: string, organizationId: string) {
        const session = await this.prisma.playgroundSession.findFirst({
            where: { id, organizationId },
        });
    ...
    async deletePlaygroundSession(id: string, organizationId: string) {
        const session = await this.prisma.playgroundSession.findFirst({
            where: { id, organizationId },
        });
